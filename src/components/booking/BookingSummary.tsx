...
const BookingSummary = ({ bookingData }: Props) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-display text-2xl text-ink mb-2">{t.booking.summary.title}</h2>
        <p className="text-ink/70 text-sm">{t.booking.summary.subtitle}</p>
      </div>
      ...
      <div className="bg-muted border p-6 space-y-4">
        <div className="flex items-start gap-3">
          ...
          <div className="text-right space-y-1">
            <p className="text-xs text-ink/60 uppercase tracking-wide">{t.booking.summary.serviceTotal}</p>
        ...
        <div className="border-t border-border pt-4 space-y-3">
          ...
          <div className="flex items-center gap-3 text-sm">
            ...
            <span className="text-ink/80">
              {t.booking.summary.duration}: {formatAppointmentDuration(treatment.bookingDurationMinutes, locale)}
            </span>
          </div>
        </div>
        ...
        <div className="border-t border-border pt-4 space-y-2">
          <div className="flex justify-between items-center text-sm">
            <span className="text-ink/80">{t.booking.summary.depositNow}</span>
            <span className="font-display text-ink">{formatPrice(depositAmountCents)}</span>
          </div>
          {balanceDue != null && servicePriceCents != null ? (
            <div className="flex justify-between items-center text-sm">
              <span className="text-ink/80">{t.booking.summary.balanceDue}</span>
      ...
      <div className="bg-accent/20 border border-accent/30 p-4 text-sm text-ink/80">
        <p className="font-medium text-ink mb-1">{t.booking.summary.policyTitle}</p>
        <ul className="space-y-1 text-xs">
          {t.booking.summary.policy.map((item) => (
            <li key={item}>• {item}</li>
          ))}
        </ul>
      </div>
